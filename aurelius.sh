#!/bin/sh
# aurelius: Read the Word of God from your terminal
...
while [ $# -gt 0 ]; do
	...
	if [ "$1" = "--" ]; then
		...
		get_data aurelius.tsv | awk -v cmd=list "$(get_data aurelius.awk)"
		exit
	elif [ "$1" = "-W" ]; then
...
if [ $# -eq 0 ]; then
	...
	while true; do
		...
		get_data aurelius.tsv | awk -v cmd=ref -v ref="$ref" "$(get_data aurelius.awk)" | ${PAGER}
	done
	exit 0
fi

get_data aurelius.tsv | awk -v cmd=ref -v ref="$*" "$(get_data aurelius.awk)" | ${PAGER}
